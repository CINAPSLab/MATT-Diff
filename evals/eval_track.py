...
BASE_DIR = os.path.join("results", "track")
OUT_JSON = os.path.join(BASE_DIR, "track_results.json")
...
def main() -> None:
    ...
    for ep in range(int(EPISODES)):
        ...
        planner = MMPlanner(
            ...
            params=dict(TRACK_STRONG),
            goal_params=dict(GOAL_COMMON),
        )

        print(f"[Track] Episode {ep}/{EPISODES} (seed={seed})")
        ...
        if SAVE_TIMESERIES_DIR is not None:
            ...
            out_npz = os.path.join(SAVE_TIMESERIES_DIR, f"track_ep{ep:03d}.npz")
            np.savez_compressed(out_npz, **dump)
            print(f"[Track] saved timeseries -> {out_npz}")
