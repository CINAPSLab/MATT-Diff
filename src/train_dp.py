...
class Config:
    ...
    wandb_project: str = "MATT-Diff"  # WandBプロジェクト名
...
def make_optim(cfg: Config, policy: DiffusionPolicyNetwork):
    ln_params = list(policy.ln_map.parameters()) + list(policy.ln_target.parameters()) + list(policy.ln_gc.parameters())
    return torch.optim.AdamW(
        [
            {"params": policy.noise_pred_net.parameters(), "lr": cfg.lr_unet, "weight_decay": cfg.weight_decay_unet},
            {"params": policy.tse.parameters(), "lr": cfg.lr_unet, "weight_decay": cfg.weight_decay_unet},
            {"params": list(policy.map_encoder.parameters()) + list(policy.map_temporal_proj.parameters()), "lr": cfg.lr_map, "weight_decay": cfg.weight_decay_map},
            {"params": ln_params, "lr": cfg.lr_unet, "weight_decay": 0.0},
        ]
    )
...
def make_sched(cfg: Config, opt: torch.optim.Optimizer, steps_per_epoch: int):
    ...
    return torch.optim.lr_scheduler.LambdaLR(opt, lr_lambda=[lr_unet, lr_unet, lr_map, lr_unet])
...
def train_loop(cfg: Config, device: torch.device, policy: DiffusionPolicyNetwork,
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for batch in dl_tr:
            ...
            lr_m = opt.param_groups[2]["lr"]

            append_csv(log_path, step, epoch, "train", loss.item(), lr_u, lr_m)
        ...
        if dl_va is not None:
            ...
            with torch.no_grad(), torch.autocast(device_type="cuda", dtype=cfg.amp_dtype):
                for batch in dl_va:
                    ...
                    cnt += 1
            val_loss = vsum / cnt
            append_csv(log_path, step, epoch, "val", val_loss, lr_u, lr_m)

            # ======= WandB ロギング (Validation) =======
            wandb.log({
                "val/loss": val_loss,
                "epoch": epoch,
                "step": step
            ...
            if val_loss < best_val:
                best_val = val_loss
                torch.save(
                    {"policy": policy.state_dict(), "cfg": cfg.__dict__, "epoch": epoch, "best_val": best_val},
                    cfg.out_dir / "best.pt",
                )
...
def main(cfg: Config):
    ...
    wandb.init(
        ...
        name=cfg.out_dir.name,  # "dp_feb16" のようなディレクトリ名がRun名になります
        config=vars(cfg)
    )
    # ============================
    ...
    dl_tr, dl_va, scaler = make_loaders(
        data_dir=cfg.data_dir, batch_size=cfg.batch_size, obs_h=cfg.obs_h, pred_h=cfg.pred_h,
        ...
        action_low=cfg.act_low, action_high=cfg.act_high,
    )

    policy = DiffusionPolicyNetwork(
        action_dim=2, obs_horizon=cfg.obs_h, pred_horizon=cfg.pred_h, map_emb_dim=cfg.map_emb_dim,
        unet_down_dims=(256, 512, 1024), unet_kernel_size=5, num_diffusion_iters=cfg.num_diffusion_iters, use_age=False,
    ).to(device)
    ...
    train_loop(cfg=cfg, device=device, policy=policy, opt=opt, sched=sched, dl_tr=dl_tr, dl_va=dl_va, log_path=log_path)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out_dir", type=Path, default=Path("output/model/dp_feb16"))
    ...
    ap.add_argument("--wandb_project", type=str, default="MATT-Diff")

    args = ap.parse_args()
    cfg = Config(**vars(args))
    cfg.act_low = tuple(args.act_low)
    cfg.act_high = tuple(args.act_high)
