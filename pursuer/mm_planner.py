...
class Coordinator:
    def __init__(
        ...
        if goal_params:
            if "random_seed" in goal_params:
                 seed = goal_params["random_seed"]
                 self.goal_selector._rng = np.random.default_rng(seed)
            
            if "softmax_temp" in goal_params:
                self.goal_selector.softmax_temp = goal_params["softmax_temp"]

            safe_keys = [
                "frontier_gain_weight", "frontier_area_weight", 
                "frontier_dist_weight", "frontier_known_weight",
                "use_simple_nearest"
            ]
            for k in safe_keys:
                if k in goal_params and hasattr(self.goal_selector, k):
                    setattr(self.goal_selector, k, goal_params[k])
    ...
    def step(
        ...
        # --- MM Planner Logic (Mode Selection) Starts Here ---
        ...
        # NOTE: Using explore_only NO_PATH trigger logic roughly here
        no_path_hit = self.no_path_count >= self.NO_PATH_TRIGGER
        
        pending_detect = (spin_trig or map_stall_hit or self.stall_count >= self.STALL_STEPS or no_path_hit)
        
        can_trigger = (self.step_count >= self.cooldown_until) and (not self.RECOVERING)
        
        # Recovery trigger moved slightly down to match flow, but logic preserved
        if pending_detect and can_trigger:
             self._start_recovery(robot_xy)

        self._check_recovery_done(robot_xy)

        # Mode Selection Logic (Keep MM Planner logic)
        if self.FORCE_EXPLORE_ONLY:
            self.mode = "EXPLORE"
        else:
        ...
        # Goal Selection Logic (Keep MM Planner logic)
        mode = self.mode
        goal = None # Initialize goal
        ...
        if not entry_mode:
            ...
            # Snap start/goal to clear zones
            ...
            if self._need_replan(robot_xy, goal):
                ...
                if plan_goal is not None:
                    ...
                    max_iter = 1400 if burst else 700  # Increased from fixed 500
                    goal_rad = 35.0 if burst else 25.0 # Slightly larger if failing
                    
                    planner = RRTStar(
                        start=plan_start,
                        goal=plan_goal,
                        grid=band, # Uses relaxed band if needed
                        max_iter=max_iter,
                        step_size=20.0, # Finer steps than 30.0
                        goal_sample_rate=0.25,
                        search_radius=115.0, # Larger search radius
        ...
        no_path_hit_now = self.no_path_count >= self.NO_PATH_TRIGGER
        if (not self.RECOVERING) and (not entry_mode) and no_path_hit_now:
            self._start_recovery(robot_xy)
